  ...
  <div class="flex flex-col items-center mx-5 mt-8">
    ...
    <div class="w-full">
      <div
        ...
        <div class="flex lg:justify-center ju items-center w-full gap-4 mb-6">
          <RouterLink :to="{ name: 'addproduct' }">
          <button
            class="flex justify-center items-center p-3 bg-gradient-to-tr from-[#ffd94d] via-[#fff099] to-[#ffd94d] shadow-md rounded-xl transition-transform duration-500 hover:scale-110">
            <p
        ...
        <h1 class="font-sarabun text-2xl font-bold mb-3">สินค้า ของฉัน</h1>
        <p class="text-purple-600 font-sarabun">0 รายการ</p>
        </div>
          ...
          <table class="w-full text-sm text-left text-gray-500 rounded-b-xl table-auto min-w-[700px]">
            <thead class="text-gray-700 uppercase bg-[#e6f2f4]">
              <tr>
                ...
                <th class="px-6 py-3 font-sarabun font-semibold lg:text-base">ราคาต้นทุน</th>
                <th class="px-6 py-3 font-sarabun font-semibold lg:text-base">ราคาส่งร้าน</th>
                <th class="px-6 py-3 font-sarabun font-semibold lg:text-base">ราคาขายหน้าร้าน</th>
                <th class="px-6 py-3 font-sarabun font-semibold lg:text-base">จำนวนคงเหลือ</th>
            ...
            <tbody v-if="currentItems.length > 0">
              <tr v-for="(item, index) in currentItems" :key="item.id || index"
                ...
                <td class="px-6 py-4 font-sarabun">{{ item.costprice }}</td>
                <td class="px-6 py-4 font-sarabun">{{ item.wholesaleprice }}</td>
                <td class="px-6 py-4 font-sarabun">{{ item.sellingprice }}</td>
                <td class="px-6 py-4 font-sarabun">{{ item.quantity }}</td>
