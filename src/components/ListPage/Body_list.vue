  ...
  <div class="flex flex-col items-center mx-5 mt-8">
    ...
    <div class="w-full">
      <div
        ...
        <h1 class="font-sarabun text-2xl font-bold mb-6 ">รายงานการลงทุน</h1>
          ...
          <table class="w-full text-sm text-left text-gray-500 rounded-b-xl table-auto min-w-[700px]">
            <thead class="text-gray-700 uppercase bg-[#e6f2f4]">
              <tr>
                ...
                <th class="px-6 py-3 font-sarabun font-semibold lg:text-base">ผู้ลงทุน</th>
                <th class="px-6 py-3 font-sarabun font-semibold lg:text-base">เบอร์โทรศัพท์</th>
                <th class="px-6 py-3 font-sarabun font-semibold lg:text-base">รูปแบบการลงทุน</th>
                <th class="px-6 py-3 font-sarabun font-semibold lg:text-base">ขนาดที่ต้องการลงทุน</th>
            ...
            <tbody v-if="currentItems.length > 0">
              <tr v-for="(item, index) in currentItems" :key="item.id || index"
                ...
                <td class="px-6 py-4 font-medium text-gray-700 font-sarabun">{{ item.invcode }}</td>
                <td class="px-6 py-4 font-sarabun">{{ item.investor}}</td>
                <td class="px-6 py-4 font-sarabun">{{ item.invTel }}</td>
                <td class="px-6 py-4 font-sarabun">{{ item.invType }}</td>
                <td class="px-6 py-4 font-sarabun">{{ item.invstatus }}</td>
              </tr>
            </tbody>
            <tbody v-else>
              <tr>
    ...
    <div class="flex justify-center items-center w-full mt-4 gap-3">
      <button class="px-4 py-2 bg-gray-300 text-gray-700 rounded-box hover:bg-gray-400 font-sarabun text-md cursor-pointer"
